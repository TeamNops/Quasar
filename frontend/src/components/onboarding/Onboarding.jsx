...
const Onboarding = () => {
  ...
  useEffect(() => {
    ...
    // We need to check if the user is in the middle of a reassessment
    // If they are, send them directly to the assessment page
    if (reassessmentInfo) {
      navigate("/assessment");
      return;
    }

    // Only redirect if onboarding is already complete AND NOT doing reassessment
    if (onboardingComplete === "true") {
      const skillAssessmentComplete = localStorage.getItem(
        "skillAssessmentComplete"
      );

      if (skillAssessmentComplete === "true") {
        // If both onboarding and assessment are done, go to dashboard
        navigate("/dashboard");
      } else {
        // If onboarding is done but assessment isn't, go to assessment
        navigate("/assessment");
      }
    }
  }, [navigate]);
